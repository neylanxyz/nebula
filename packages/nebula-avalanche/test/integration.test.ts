...
describe('nebula-avalanche wiring', () => {
  it('exports correct Avalanche Fuji constants', () => {
    expect(NEBULA_CONTRACT_ADDRESS).toBe('0x254d1290A8f977dc2bABbbf979CC86A7AC4a83ca');
    expect(VERIFIER_CONTRACT_ADDRESS).toBe('0xEf4feB1ab6fcCF29ac1217A500Db187dE2758ac1');
  ...
  it('exports NEBULA_POOL_ABI with expected functions', () => {
    const functionNames = NEBULA_POOL_ABI
  ...
  it('exports VERIFIER_ABI with verify function', () => {
    const verifyFn = VERIFIER_ABI.find(
      (item) => item.type === 'function' && 'name' in item && item.name === 'verify',
    );
