    ...
    <div className="space-y-4 rounded-xl border border-gray-800 bg-gray-900 p-6">
      ...
      {txHash && (
        <p className="text-sm text-emerald-400">
          Tx:{' '}
          <a href={`https://testnet.snowtrace.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="underline">
            {txHash.slice(0, 10)}...
          </a>
        </p>
      )}
      ...
      {note && (
        <div className="space-y-2 rounded-lg border border-yellow-600/50 bg-yellow-900/10 p-4">
          <p className="text-sm font-bold text-yellow-400">
            IMPORTANTE: Copie e salve esta nota! Necessária para sacar o prêmio.
          </p>
          <textarea readOnly value={note} className="w-full rounded bg-gray-800 p-2 font-mono text-xs text-gray-300" rows={3} />
          <button onClick={copyNote} className="w-full rounded-lg border border-yellow-600 px-4 py-2 text-sm font-semibold text-yellow-400 transition hover:bg-yellow-900/20">
